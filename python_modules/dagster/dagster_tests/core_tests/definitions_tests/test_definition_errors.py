...
def test_bad_output_definition():
    with pytest.raises(
        DagsterInvalidDefinitionError,
        match=re.escape(
            'Invalid type: dagster_type is not an instance of "type", got foo'
        )
    ):
    ...
    with pytest.raises(
        ...
        match=re.escape(
            ...
            '<dagster_tests.core_tests.definitions_tests.test_definition_errors.test_bad_output_'
            'definition.<locals>.Exotic object'
        )
    ):
